...
<body>
	...
	<div align="left">
		Usuario: {{ $SOs[0]->nameCustomer }}
	</div>

	<div align="right"> 
		Pedidos desde: {{ date("d", strtotime($since)) }}/{{ date("m", strtotime($since)) }}/{{ date("Y", strtotime($since)) }}, hasta: {{ date("d", strtotime($until)) }}/{{ date("m", strtotime($until)) }}/{{ date("Y", strtotime($until)) }}<br>
	</div>
	...
	<table>
		...
		<tbody>
            ...
            <tr>
                ...
                <td>
                	{{ date("d", strtotime($q->SODateSale)) }}/{{ date("m", strtotime($q->SODateSale)) }}/{{ date("Y", strtotime($q->SODateSale)) }} {{ date("H", strtotime($q->SODateSale)) }}:{{ date("i", strtotime($q->SODateSale)) }}:{{ date("s", strtotime($q->SODateSale)) }}
                </td>
                <td>{{ $q->name }}</td>
            </tr>
            @endforeach
