...
<head>
	...
	<title>Proveedor por fecha</title>
	<style type="text/css">
...
<body>
	...
	<div align="right"> 
		Proveedores desde: {{ date("d", strtotime($since)) }}/{{ date("m", strtotime($since)) }}/{{ date("Y", strtotime($since)) }}, hasta: {{ date("d", strtotime($until)) }}/{{ date("m", strtotime($until)) }}/{{ date("Y", strtotime($until)) }}<br>
	</div>
	...
	<table>
		...
		<tbody>
			@foreach ($provider as $e)
                <tr>
                    <td>{{ $e->codeProvider }}</td>
                    <td>{{ $e->nameProvider }}</td>
                    <td>{{ $e->addressProvider }}</td>
                    <td>{{ $e->phoneProvider }}</td>
                    <td>{{ $e->emailProvider }}</td>
                </tr>
            @endforeach
		</tbody>
	</table>
